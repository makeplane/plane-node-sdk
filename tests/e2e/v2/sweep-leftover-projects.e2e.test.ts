...
maybe("v2 sweepLeftoverProjects age gate (live)", () => {
  ...
  afterEach(async () => {
    if (project) {
      await client.v2.workspaces.projects.delete(env.workspaceSlug, project.id).catch(() => undefined);
    }
  });
  ...
  it("does NOT sweep a project that only looks a few minutes old (the incident this guards against)", async () => {
    ...
    await expect(client.v2.workspaces.projects.retrieve(env.workspaceSlug, project.id)).resolves.toMatchObject({
      id: project.id,
    });
  });
  ...
  it("DOES sweep a project once it is provably older than the threshold (the guard still guards)", async () => {
    ...
    await expect(client.v2.workspaces.projects.retrieve(env.workspaceSlug, project.id)).rejects.toThrow();
  });
});
