...
const makeResource = () => new Stickies(new V2Transport(new Configuration({ baseUrl: BASE, apiKey: "secret" })));
...
describe("Stickies (v2)", () => {
  it("lists stickies, optionally filtered by owner", async () => {
    ...
    const page = await makeResource().list(SLUG, { owner_id: "u1" });
  ...
  it("creates a blank sticky with an empty body", async () => {
    ...
    const created = await makeResource().create(SLUG, {});
  ...
  it("creates then patches", async () => {
    ...
    const created = await resource.create(SLUG, { name: "Note" });
    const updated = await resource.update(SLUG, created.id, { color: "#f00" });
  ...
  it("rejects an unknown field before making the request", async () => {
    ...
    await expect(makeResource().list(SLUG, { fields: ["priority"] as never })).rejects.toThrow(
