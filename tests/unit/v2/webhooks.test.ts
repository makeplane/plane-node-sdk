...
const makeWebhooks = () =>
  new Webhooks(new V2Transport(new Configuration({ baseUrl: BASE, apiKey: "secret" })), { slug: SLUG });
...
describe("Webhooks (v2)", () => {
  ...
  it("lists, retrieves, updates, deletes a webhook", async () => {
    ...
    const fetched = await webhooks.retrieve("w1");
    expect(fetched.url).toBe("https://hooks.example.com");
    const updated = await webhooks.update("w1", { is_active: false });
    expect(updated.is_active).toBe(false);
    await expect(webhooks.delete("w1")).resolves.toBeUndefined();
  });
  ...
  it("create returns secret_key once; list/retrieve never carry it", async () => {
    ...
    const webhooks = makeWebhooks();
    const created = await webhooks.create(write);
  ...
  it("regenerate hits POST {pk}/regenerate/ and returns a fresh secret_key", async () => {
    ...
    const regenerated = await makeWebhooks().regenerate("w1");
  ...
  it("rejects an unknown order_by before making the request", async () => {
    await expect(makeWebhooks().list({ order_by: "url" as never })).rejects.toThrow(
  ...
  it("filters by is_active/url/search", async () => {
    ...
    await makeWebhooks().list({ is_active: true, url: "hooks.example.com", search: "prod" });
    expect(scope.isDone()).toBe(true);
  });
  ...
  it("finds by name", async () => {
    ...
    expect((await makeWebhooks().findByName("Prod notifier")).id).toBe("w1");
    expect(scope.isDone()).toBe(true);
  });
});
