...
const makeResource = () => new Intakes(new V2Transport(new Configuration({ baseUrl: BASE, apiKey: "secret" })));
...
describe("Intakes (v2)", () => {
  it("lists intake work items for a project", async () => {
    ...
    const page = await makeResource().list("acme", "ENG", { status: -2 });
  ...
  it("creates then patches a triage decision in one call", async () => {
    ...
    const created = await resource.create("acme", "ENG", { name: "Bug report" });
    expect(created.status).toBe(-2);

    const updated = await resource.update("acme", "ENG", created.id, { status: 1 });
  ...
  it("retrieves and deletes", async () => {
    ...
    const fetched = await makeResource().retrieve("acme", "ENG", "1");
    expect(fetched.name).toBe("Bug report");

    await expect(makeResource().delete("acme", "ENG", "1")).resolves.toBeUndefined();
  ...
  it("passes through a valid order_by", async () => {
    ...
    await makeResource().list("acme", "ENG", { order_by: "-created_at" });
  ...
  it("rejects an unknown order_by before making the request", async () => {
    ...
    await expect(makeResource().list("acme", "ENG", { order_by: "priority" as never })).rejects.toThrow(
