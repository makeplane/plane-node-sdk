...
describe("WorkItems.attachments (v2)", () => {
  ...
  const make = () => new Attachments(makeTransport());
  ...
  it("lists, narrows fields, and rejects an unknown field", async () => {
    ...
    const page = await make().list(SLUG, PROJECT, WORK_ITEM);
    expect(page.data[0].name).toBe("file.png");

    await expect(make().list(SLUG, PROJECT, WORK_ITEM, { fields: ["nope" as never] })).rejects.toThrow(/nope/);
  });
  ...
  it("is a two-step upload: create returns credentials, update confirms is_uploaded", async () => {
    ...
    const created = await attachments.create(SLUG, PROJECT, WORK_ITEM, createBody);
    expect(created.is_uploaded).toBe(false);
    expect(created.asset_url).toBeTruthy();

    const confirmed = await attachments.update(SLUG, PROJECT, WORK_ITEM, "a1", { is_uploaded: true });
    expect(confirmed.is_uploaded).toBe(true);

    await expect(attachments.delete(SLUG, PROJECT, WORK_ITEM, "a1")).resolves.toBeUndefined();
  });

  it("retrieves one attachment", async () => {
    nock(BASE).get(`${collection}a1/`).reply(200, { id: "a1" });
    expect((await make().retrieve(SLUG, PROJECT, WORK_ITEM, "a1")).id).toBe("a1");
  });
});
...
describe("WorkItems.links (v2)", () => {
  ...
  it("creates, retrieves, updates, deletes a link", async () => {
    ...
    const links = make();
    const created = await links.create(SLUG, PROJECT, WORK_ITEM, write);
    expect(created.id).toBe("l1");
    const fetched = await links.retrieve(SLUG, PROJECT, WORK_ITEM, "l1");
    expect(fetched.url).toBe(write.url);
    const updated = await links.update(SLUG, PROJECT, WORK_ITEM, "l1", { title: "Renamed" });
    expect(updated.title).toBe("Renamed");
    await expect(links.delete(SLUG, PROJECT, WORK_ITEM, "l1")).resolves.toBeUndefined();
  });
  ...
  it("lists with a title/url filter", async () => {
    ...
    await make().list(SLUG, PROJECT, WORK_ITEM, { title: "Example" });
...
describe("WorkItems.worklogs (v2)", () => {
  ...
  const make = () => new WorkLogs(makeTransport());
  ...
  it("creates, retrieves, updates, deletes a worklog", async () => {
    ...
    const worklogs = make();
    const created = await worklogs.create(SLUG, PROJECT, WORK_ITEM, write);
    expect(created.duration).toBe(60);
    const fetched = await worklogs.retrieve(SLUG, PROJECT, WORK_ITEM, "w1");
    expect(fetched.description).toBe("Investigated");
    const updated = await worklogs.update(SLUG, PROJECT, WORK_ITEM, "w1", { duration: 90 });
    expect(updated.duration).toBe(90);
    await expect(worklogs.delete(SLUG, PROJECT, WORK_ITEM, "w1")).resolves.toBeUndefined();
  });
  ...
  it("encodes the logged_by expand", async () => {
    const scope = nock(BASE)
      ...
      .reply(200, { data: [], pagination: { style: "offset" } });
    await make().list(SLUG, PROJECT, WORK_ITEM, { expand: ["logged_by"] });
    expect(scope.isDone()).toBe(true);
  });

  it("rejects an expand value the operation doesn't offer", async () => {
    await expect(make().list(SLUG, PROJECT, WORK_ITEM, { expand: ["actor" as never] })).rejects.toThrow(
...
describe("WorkItems.activities (v2, read-only)", () => {
  ...
  const make = () => new Activities(makeTransport());
  ...
  it("lists and retrieves, following pages via iterate", async () => {
    ...
    expect((await make().retrieve(SLUG, PROJECT, WORK_ITEM, "act1")).id).toBe("act1");
  });
  ...
  it("iterates across pages", async () => {
    ...
    const seen: string[] = [];
    for await (const row of make().iterate(SLUG, PROJECT, WORK_ITEM)) seen.push(row.id);
