...
describe("WorkspaceFeatures (v2)", () => {
  const make = () => new WorkspaceFeatures(makeTransport(), { slug: "acme" });
  ...
  it("retrieves at the bare collection URL, no id segment", async () => {
    ...
    const feature = await make().retrieve();
  ...
  it("patches at the bare collection URL", async () => {
    ...
    const updated = await make().update({ is_wiki_enabled: false });
...
describe("ProjectFeatures (v2)", () => {
  const make = () => new ProjectFeatures(makeTransport(), { slug: "acme", project_id: "ENG" });
  ...
  it("patches at the bare project-scoped collection URL", async () => {
    ...
    const updated = await make().update({ is_epic_enabled: false });
