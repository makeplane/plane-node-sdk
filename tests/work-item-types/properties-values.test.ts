...
export async function testWorkItemPropertiesValues() {
  ...
  if (!workItemId || !propertyId) {
    throw new Error('workItemId and propertyId are required to test work item properties and values');
  }

  const workItemPropertyValue = await updateWorkItemPropertyValue(client, workspaceSlug, projectId, workItemId, propertyId);
...
async function updateWorkItemPropertyValue(
  ...
  const workItemPropertyValue = await client.workItemProperties.values.create(workspaceSlug, projectId, workItemId, propertyId, {
