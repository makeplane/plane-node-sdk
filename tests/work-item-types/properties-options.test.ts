...
import { TTextSettings, WorkItemProperty, WorkItemPropertyOption, WorkItemPropertySettings, WorkItemPropertyType } from '../../src/models/WorkItemProperty';
...
export async function testWorkItemTypesPropertiesAndOptions() {
  ...
  if (!workItemTypeId) {
    throw new Error('workItemTypeId is required to test work item properties and options');
  }
  ...
  const workItemTypeProperty = await createWorkItemTypeProperty(client, workspaceSlug, projectId, workItemTypeId, 'TEXT', {
    display_format: 'single-line',
  });
  console.log('Created work item type property: ', workItemTypeProperty);

  const retrievedWorkItemTypeProperty = await retrieveWorkItemTypeProperty(client, workspaceSlug, projectId, workItemTypeId, workItemTypeProperty.id);
  console.log('Retrieved work item type property: ', retrievedWorkItemTypeProperty);

  const updatedWorkItemTypeProperty = await updateWorkItemTypeProperty(client, workspaceSlug, projectId, workItemTypeId, workItemTypeProperty.id);
  ...
  const optionWorkItemTypeProperty = await createWorkItemTypeProperty(client, workspaceSlug, projectId, workItemTypeId, 'OPTION');
  ...
  const createdWorkItemPropertyOption = await createWorkItemPropertyOption(client, workspaceSlug, projectId, optionWorkItemTypeProperty.id);
  console.log('Created work item property option: ', createdWorkItemPropertyOption);

  const retrievedWorkItemPropertyOption = await retrieveWorkItemPropertyOption(client, workspaceSlug, projectId, optionWorkItemTypeProperty.id, createdWorkItemPropertyOption.id);
  console.log('Retrieved work item property option: ', retrievedWorkItemPropertyOption);

  const updatedWorkItemPropertyOption = await updateWorkItemPropertyOption(client, workspaceSlug, projectId, optionWorkItemTypeProperty.id, createdWorkItemPropertyOption.id);
  console.log('Updated work item property option: ', updatedWorkItemPropertyOption);

  await deleteWorkItemPropertyOption(client, workspaceSlug, projectId, optionWorkItemTypeProperty.id, createdWorkItemPropertyOption.id);
...
async function deleteWorkItemTypeProperty(
  ...
  await client.workItemProperties.del(workspaceSlug, projectId, workItemTypeId, workItemPropertyId);
}
...
async function createWorkItemPropertyOption(
  ...
  const workItemTypePropertyOption = await client.workItemProperties.options.create(workspaceSlug, projectId, workItemPropertyId, {
...
async function retrieveWorkItemPropertyOption(
  ...
  const workItemTypePropertyOption = await client.workItemProperties.options.retrieve(workspaceSlug, projectId, workItemPropertyId, workItemPropertyOptionId);
  return workItemTypePropertyOption;
}
...
async function updateWorkItemPropertyOption(
  ...
  const workItemTypePropertyOption = await client.workItemProperties.options.update(workspaceSlug, projectId, workItemPropertyId, workItemPropertyOptionId, {
...
async function deleteWorkItemPropertyOption(
  ...
  await client.workItemProperties.options.del(workspaceSlug, projectId, workItemPropertyId, workItemPropertyOptionId);
}
